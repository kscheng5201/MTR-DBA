...
# Created: 2023-02-23
# Updated: 2023-07-11
# Note: Snapshot 計算以前一小時版本為主，因當前檔案雖 SUCCESS 但未完整
#####################################################################
...
## 資料量成長報告
date=`date -d '-1 hour' +%Y%m%d.%H%M`
last_day=`date -d "-1 hour -7 day" +%Y%m%d.%H%M`

today_size=$(curl -X GET `hostname`:9200/_snapshot/backup/backup-$date/_status?pretty | tac | tac | grep size_in_bytes | head -n 1 | awk '{print $3}' | cut -d , -f 1)
