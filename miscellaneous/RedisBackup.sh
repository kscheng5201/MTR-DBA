...
# Created: 2022-12-05
# Updated: 2023-01-07
# Note: 修改 back server 的資料位置改為 /efsbackup
##################################################
...
backup_location=/usr/local/redis-cluster
...
if [ $flag != "0" ]; then
    ...
    gzip -c $backup_location/data/$dump_file > $backup_dir/`echo $redis_host | cut -d . -f 4`-$dump_file-$backup_time.gz
    ...
    gzip -c $backup_location/data/$append_file > $backup_dir/`echo $redis_host | cut -d . -f 4`-$append_file-$backup_time.gz
...
scp -i /home/gok/.ssh/gok_rsa -r $backup_dir gok@${backup_server}:/efsbackup/redis 
scp -i /home/gok/.ssh/gok_rsa -r $backup_location/backup/$backup_log gok@${backup_server}:/efsbackup/redis 
