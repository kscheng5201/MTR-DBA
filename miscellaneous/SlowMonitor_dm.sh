...
head_now=$(sudo cat -n $slow_dir | grep '# Time' | grep `date -d "8 hour ago 1 minute ago" +"%Y-%m-%dT%H:%M"` | head -n 1 | cut -d '#' -f 1)
...
then 
    ...
    # Count Only 後台注單查詢的導出功能所引發之Slow Query
    gl_game_bet=`sudo tail -n +$head_now $slow_dir | grep 'and status = 1 ) order by bet_date asc, order_id asc LIMIT' | wc -l`

    if [[ $SQL_TOTAL = $gcprds ]]; 
    then 
        # The number for Zabbix dashboard
        echo 0
    elif [[ $SQL_TOTAL = $gl_game_bet ]];
    then 
        # The number for Zabbix dashboard
        echo 0
    else 
        ...
        if [[ $vIndicator = "freq" ]]; 
        then 
            echo $(($SQL_TOTAL - $gcprds - $gl_game_bet))
...
if [[ "$date" =~ "08:00" ]];
then
    p=$(sudo grep '# Time' $slow_dir | grep `date -d "1 day ago" +"%Y-%m-%d"` | wc -l)
    msg=$(echo "昨天 `date -d "1 day ago" +"%Y-%m-%d"` 共有 $p 筆 SlowQuery")
    curl -X POST "https://api.telegram.org/bot${token}/sendMessage" -d "chat_id=${chat}&text=${title} at [[`hostname`]] 
