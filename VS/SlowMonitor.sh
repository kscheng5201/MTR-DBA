...
then 
    ...
    then 
        ...
        f=`sudo grep -c $last_minute $slow_dir`
        echo $f
        ...
        then 
            ...
            then 
                ...
                if [[ -s $data_dir/up30f0m_$last_minute.log ]];
                then 
    ...
    then
        ...
        t=`sudo grep -A2 $last_minute $slow_dir | grep Query_time | awk '{print $3}' | sort -n | tail -1`
        printf "%.2f \n" $t

        more60=`sudo grep -A2 $last_minute $slow_dir | grep Query_time | awk '{print $3}' | sort -n | tail -1 | cut -d . -f 1`
        if [[ $more60 -ge 60 ]]; 
        then 
            sudo sh /etc/dba/more60s.sh $last_minute
        fi
    ...
    then
        ...
        r=`sudo grep -A2 $last_minute $slow_dir | grep Rows_sent | awk '{print $7}' | sort -n | tail -1`
        echo $r
...
then 
    ...
    ## 若產生出來的檔案為空，則刪除；不然則傳送
    if [[ -s $data_dir/up30f10m_$last_minute.log ]];
    then 
