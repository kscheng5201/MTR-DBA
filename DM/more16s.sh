...
then
    ...
    ## 混有秒數 < 16 秒的 SQL
        ...
        ## 取得關鍵時間戳之後的所有時間戳行數
        sudo cat -n $slow_dir | awk "/$last/,0" | grep Time | awk '{print $1}' > $data_dir/key.log
...
## 刪除空白行
sudo awk NF $data_dir/output.log > $data_dir/`hostname | cut -d - -f 3-4`_more16s_${last}.log 2> /dev/null

## 若產生出來的檔案為空，則刪除；不然則傳送
if [[ $(sudo ls -al $data_dir/`hostname | cut -d - -f 3-4`_more16s_${last}.log | awk '{print $5}') -eq 0 ]];
then 
