...
# Created: 2022-11-22
# Updated: 2023-04-24
# Note: 修改備份傳送位置到 /home/gok/backups
#############################################
...
## 刪除過期備份
if [[ `ls $backup_location | grep [0-9] | wc -l` -gt 5 ]];
then 
    last_5th=`ls $backup_location | grep [0-9] | tail -5 | head -1`
    delete_to=`ls $backup_location | grep [0-9] | grep -B111 "$last_5th"`

echo last5 = $last_5th
echo delete_to = $delete_to
...
scp -i /home/gok/.ssh/gok_rsa -r $backup_dir gok@${backup_server}:/home/gok/backups/rabbitmq
scp -i /home/gok/.ssh/gok_rsa -r $backup_log gok@${backup_server}:/home/gok/backups/rabbitmq
